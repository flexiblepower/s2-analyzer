...
function Sidebar() {
    ...
    return (
        <div className="overflow-auto flex absolute transition-colors">
            <div style={{width: `${width / 16}rem`, height: window.innerHeight * 0.86}} className="border-tno-blue border-r bg-base-gray">
                ...
                {parser.getErrors().length != 0 &&
                    parser.getErrors().map((s: string, index) => {
                        return (
                            <pre key={index} className={"text-white overflow-auto bg-base-gray"} style={{maxWidth: width, overflowX: "hidden"}}>
                                {s}
                            </pre>
                        );
                    })
            ...
            <div className="w-2 cursor-col-resize" onMouseDown={() => {isResized.current = true;}}/>
        </div>
    );
}
