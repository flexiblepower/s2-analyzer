...
function PowerForecastGraph(props: Props) {
    const getIndex = (com: string, labels:CommodityQuantity[], maxElements:number)=> {
    ...
    const getExpectedMatrix = (labels: CommodityQuantity[]) => {
        ...
        const matrix = Array.from({length: maxElements}, () => Array.from({length: length}, () => -1));
        for (let i = 0; i < length; i++) {
            for (let j = 0; j < props.data[i].power_values.length; j++) {
                const row = getIndex(props.data[i].power_values[j].commodity_quantity, labels, maxElements)
                if (row!=-1) {
                    let column = 0
    ...
    const collectGraphData = () => {
        const {matrix, labs} = getExpectedMatrix(Object.values(CommodityQuantity))
        ...
        const labs2 = labs.map(str=> {const w=str.split('.'); return w.map(e=>e.substring(0,2)).join('.')})
