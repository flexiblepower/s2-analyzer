...
describe("Parser", () => {
  ...
  describe("getMessages()", () => {
    ...
    test("UT-17: should be able to change the status of the message if the message has subject_message_id", () => {
      ...
      expect(parser.getMessages()).toContainEqual(
        parser.messageMap.find(
          (message) =>
    ...
    test("UT-19: should be able to change the status of the message if the message has object_id", () => {
      ...
      expect(parser.getMessages()).toContainEqual(
        parser.messageMap.find(
          (message) =>
            message.status ===
              "revoked by message_id: " + message1.message_id &&
            message.message_id === message2.message_id
        )
      );
    });
  ...
  describe("parseLogFile()", () => {
    global.window = { showOpenFilePicker: jest.fn() } as unknown as Window &
      typeof globalThis;
      ...
      const fileHandles = [
        {
          getFile: jest.fn().mockResolvedValue({
            text: jest.fn().mockResolvedValue("Line1\nLine2\n"),
          }),
        },
        {
          getFile: jest.fn().mockResolvedValue({
            text: jest.fn().mockResolvedValue("Line3\nLine4\nLine5\n"),
          }),
        },
      ];
    ...
    test("UT-21: should return the messages from getMessages", async () => {
      ...
      const fileHandles = [
        {
          getFile: jest.fn().mockResolvedValue({
            text: jest.fn().mockResolvedValue(JSON.stringify(message1)),
          }),
        },
        {
          getFile: jest.fn().mockResolvedValue({
            text: jest.fn().mockResolvedValue(JSON.stringify(message2)),
          }),
        },
      ];
  ...
  describe("extractHeader()", () => {
    ...
    test("UT-40: should return the message header for valid message", () => {
      ...
      expect(message?.message_id).toEqual(
        "00ef6f72-257c-46a5-a656-07887903eb09"
      );
  ...
  describe("extractField()", () => {
    ...
    test("UT-48: should return the wanted field value", () => {
      ...
      expect(parser.extractField(line, fieldName3)).toEqual(
        "{'message_type': 'Handshake', 'message_id': '7dd55ca8-f15c-4cad-adf5-154d11a9a2e1', 'role': 'RM'}"
      );
    });
  });
  ...
  describe("castToMessageType()", () => {
    ...
    test("UT-51: should return the Handshake message type", () => {
      ...
      expect(
        parser.castToMessageType(messageString, messageString.length)
